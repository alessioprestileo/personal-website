...
app.get('/resume/Alessio_Prestileo_Resume.jpg', (_req, res) => {
  res.setHeader(
    'Cache-Control',
    'no-store, no-cache, must-revalidate, max-age=0',
  );
  res.setHeader('Pragma', 'no-cache');
  res.setHeader('Expires', '0');
  res.sendFile(join(resumeDir, 'Alessio_Prestileo_Resume.jpg'));
});
app.get('/resume/scrollScript.js', (_req, res) => {
  res.setHeader(
    'Cache-Control',
    'no-store, no-cache, must-revalidate, max-age=0',
  );
  res.setHeader('Pragma', 'no-cache');
  res.setHeader('Expires', '0');
  res.sendFile(join(resumeDir, 'scrollScript.js'));
});
...
app.get('/api/github-search/search', async (req, res) => {
  ...
  if (!accessToken) {
    return res.redirect(
      `/projects/github-search/logout?auth_status=access_token_not_found`,
    );
  }
});
